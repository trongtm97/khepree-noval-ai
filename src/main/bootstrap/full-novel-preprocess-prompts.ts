...
export function buildFullNovelPreprocessPrompt(input: PreprocessPromptInput): string {
  ...
  return [
    ...
    `- Include: sourceTitle, targetTitle, alternativeTitles, author, genre, language pair (${source.nativeName} → ${target.nativeName}), tone, audience notes.`,
    '',
    '### 01_TRANSLATION_RULES.md',
    `- Start with "# ${input.projectTitle} — Translation rules"`,
    `- State: Source language: ${source.nativeName} (${input.sourceLanguage}).`,
    `- State: Target language: ${target.nativeName} (${input.targetLanguage}).`,
