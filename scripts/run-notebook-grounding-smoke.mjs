...
const enabled = ['1', 'true', 'yes'].includes(
  String(
    process.env.NOVELTRANS_NOTEBOOK_GROUNDING_SMOKE ??
      process.env.NOVELTRANS_GOOGLE_SMOKE ??
      '',
  )
    .trim()
    .toLowerCase(),
...
5. set NOVELTRANS_NOTEBOOK_GROUNDING_SMOKE=1
...
const result = spawnSync(
  ...
  ['vitest', 'run', '--config', 'vitest.notebook-grounding-smoke.config.ts'],
  {
    ...
    env: {
      ...process.env,
      NOVELTRANS_NOTEBOOK_GROUNDING_SMOKE: '1',
    },
